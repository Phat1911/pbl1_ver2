...
void inTomTatLyThuyet(void) {
    ...
    printf("3. Mot do thi lien thong co cay phu khi ta chon duoc n-1 canh ma van noi het n dinh.\n");
    printf("4. Duyet theo chieu ngang (BFS) di tung lop: tham cac dinh gan truoc, xa sau.\n");
}

void bfsCayPhu(int **a, int n, int start, int *parent) {
    int front = 0;
    int rear = 0;
    int i;
...
void inKetQuaCayPhu(int *parent, int n, int start) {
    ...
    printf("\nCac canh trong cay phu theo chieu ngang:\n");
    for (i = 0; i < n; i++) {
        if (parent[i] != -1) {
            printf("- %d -> %d\n", parent[i] + 1, i + 1);
        }
    ...
    if (demDinhDaNoi(parent, n, start) == n) {
        printf("=> Da tao duoc cay phu BFS vi do thi lien thong tu dinh bat dau.\n");
    } else {
...
void inGoiYSuDung(int n) {
    printf("\nHUONG DAN NHANH\n");
    printf("- Ta se co 1 ma tran %d x %d. Mỗi hang la thong tin noi cua 1 dinh.\n", n, n);
    printf("- Dinh duoc danh so tu 1 den %d.\n", n);
    printf("- Vi du: neu hang 1 cot 3 bang 1, nghia la dinh 1 noi voi dinh 3. Nguoc lai Neu hang 1 cot 3 bang 0 -> 1 khong noi voi dinh 3\n");
...
int nhapDinhBatDau(int n) {
    int start;
    printf("Nhap dinh bat dau de tao cay phu BFS (1..%d): ", n);
    scanf("%d", &start);
    return start - 1;
}
